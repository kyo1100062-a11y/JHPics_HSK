...
interface Custom2OriginalRatioPortraitLayoutProps {
  slots: ImageSlotType[]
...
function Custom2OriginalRatioPortraitLayout({
  slots,
  onImageSelect,
  onDelete,
  onEdit,
...
}: Custom2OriginalRatioPortraitLayoutProps) {
  // 커스텀2 템플릿: 사용자가 자유롭게 슬롯을 배치
  // 최대 16개 슬롯 지원
  // 커스텀과 동일한 레이아웃 구조
  ...
  // 세로형 4행(슬롯 10-12개) 및 5행(슬롯 13-15개)에서 텍스트 영역 보호를 위한 최소 높이 계산
  // 텍스트 영역 21px + 이미지 최소 높이 100px = 121px (여유 있게 130px)
  const isProblematicRows = 
    (rows === 4 && slots.length >= 10 && slots.length <= 12) ||
    (rows === 5 && slots.length >= 13 && slots.length <= 15)
            ...
            return (
              <div key={slot.id} className="relative" style={{ width: slotWidth, flex: '1 1 0', minHeight: isProblematicRows ? minSlotHeight : 'auto' }}>
                <ImageSlot
                  ...
                  croppedImageUrl={slot.croppedImageUrl}
...
export default Custom2OriginalRatioPortraitLayout
