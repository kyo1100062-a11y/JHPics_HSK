...
- 파일명 규칙: "제목"_"사업명"_"보조사업자"_페이지N.jpg 형태로 저장됨
...
### 9.4 출력 경로 지정 기능 (신규)
#### 9.4.1 기능 설명
- 출력 버튼(PDF 내보내기 또는 JPEG 내보내기) 클릭 시 저장 경로를 선택할 수 있음
- File System Access API를 사용하여 디렉토리 선택
- 선택한 경로에 파일 저장

#### 9.4.2 동작 규칙
- 출력 버튼 클릭 시마다 항상 경로 선택 대화상자 표시
- 사용자가 경로를 선택하면 해당 경로에 파일 저장
- 선택한 경로는 IndexedDB에 저장되어 다음에 기본값으로 제안 가능 (현재는 매번 선택)
- 사용자가 경로 선택을 취소하면 기존 방식(브라우저 다운로드)으로 진행

#### 9.4.3 브라우저 지원
- Chrome (최신 버전)
- Edge (최신 버전)
- Firefox/Safari는 제한적 지원 (기존 다운로드 방식으로 대체)

#### 9.4.4 파일 저장 방식
- PDF: Blob으로 변환하여 선택한 디렉토리에 저장
- JPEG: 각 페이지별로 Blob으로 변환하여 선택한 디렉토리에 저장
- 파일명 규칙: 기존 파일명 규칙과 동일
...
▷ 출력 파일명 규칙
- 출력 시 파일명은 "제목"_"사업명"_"보조사업자" 형태로 저장됨
